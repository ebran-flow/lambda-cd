...
def lambda_handler(event, context):
    ...
    def is_type(data):
        ...
        if ('CASH_IN' in data or 'CASH_OUT' in data or 'TRANSFER' in data or 'EXTERNAL' in data or 'WITHDRAW_MONEY' in data or 'DEPOSIT_MONEY' in data):
            return True
        else:
            return False
            ...
            for value in row:
                if isinstance(value, str):
                    ...
                    if 'ID' in value or 'TYPE' in value or 'SUBSCRIBER' in value or 'FID' in value:
                        ...
                        break
        # return sheet
    def remove_large_rows(sheet):
        for column in sheet.columns:
            sheet = sheet[sheet[column].str.split().str.len().lt(7)]
        return sheet
    def remove_wrong_balances(sheet):
        sheet = sheet[sheet['balance'].str.split().str.len().lt(3)]
        return sheet
    ...
    def clean_table(sheet):
        ...
        # sheet = remove_wrong_balances(sheet)
    ...
    def find_column_names(sheet):
        has_date, has_type, has_amount = False, False, False
        ...
        for column in sheet.columns:
            ...
            date_occurences, type_occurences, amount_occurences = 0, 0, 0
            for i in range(0, l):
            ...
            if maximum_occurence != 'none':
                if maximum_occurence == 'txn_date': has_date = True
        ...
        print(has_date, has_type, has_amount)
        if has_date and has_type and has_amount:
        # if True:
    ...
    def find_balance_column(sheet):
        ...
        for column in sheet.columns:    
            ...
            len_of_column = len(balance_list) - 2
            balance_occurences = 0
            l = MAX_COLS_TO_CHECK if (MAX_COLS_TO_CHECK > 0 and len_of_column > MAX_COLS_TO_CHECK) else len_of_column
            balance_occurences = find_balance_occurences(amount_list, balance_list, l)
            # sheet = sheet.drop(columns = ['temp_column'], axis=1)
            
            if balance_occurences > l - 10:
                # Allow upto 10 records if the amount was cleaned up to zero if seal comes or number is recognised as alphabets 
                has_balance = True
        ...
        if has_balance:
            sheet = sheet.drop(columns = ['temp_column'], axis=1)
            sheet = sheet[['txn_date', 'txn_type', 'data_prvdr_cust_id', 'amount', 'balance']].copy()
            return sheet
        else:
            ## SEND EMAIL
